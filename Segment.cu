...
void SDL::Segment::runSegmentAlgo(SDL::SGAlgo algo, SDL::LogLevel logLevel)
{
    ...
    {
        printf("Warning: Unrecognized segment algorithm!\n");
        return;
    }
}
...
//    const std::vector<unsigned int>& connectedModuleDetIds = moduleConnectionMap.getConnectedModuleDetIds(innerLowerModule.detId());
    ...
    const float sdSlope = std::asin(min(outerMiniDoubletAnchorHitRt * k2Rinv1GeVf / ptCut, sinAlphaMax));
...
//    float sdRt = innerMiniDoubletAnchorHit.rt();
    ...
    float dAlphaCutValues[3];
    dAlphaThreshold(innerMiniDoublet,outerMiniDoublet,dAlphaCutValues);
    float dAlpha_compat_inner_vs_sg = dAlphaCutValues[0];
    float dAlpha_compat_outer_vs_sg = dAlphaCutValues[1];
    float dAlpha_compat_inner_vs_outer = dAlphaCutValues[2];
...
/*    bool found = false;
    ...
    const float sdSlope = asin(min(outerMiniDoubletAnchorHitRt * k2Rinv1GeVf / ptCut, sinAlphaMax));
//     const float sdPVoff = 0.1f / outerMiniDoubletAnchorHitRt;
    // const float dzDrtScale = std::tan(sdSlope) / sdSlope; //FIXME: need approximate value
    const float pixelPSZpitch = 0.15;
    const float strip2SZpitch = 5.0;
    ...
    float rtLo = max(innerMiniDoubletAnchorHitRt * (1.f + dz / (innerMiniDoubletAnchorHitZ + dLum) * drtDzScale) - rtGeom, innerMiniDoubletAnchorHitRt - 0.5f * rtGeom); //rt should increase
...
void SDL::Segment::dAlphaThreshold(const SDL::MiniDoublet &innerMiniDoublet, const SDL::MiniDoublet &outerMiniDoublet, float* dAlphaValues)
{
...
//    const float sdPVoff = 0.1f/segmentDr;
    ...
    const float drdzinner =  (innerLowerModule.moduleLayerType() == SDL::Module::Pixel) ?  (innerLowerModule.partnerModule())->getDrDz() : innerLowerModule.getDrDz(); 
    const float drdzouter =  (outerLowerModule.moduleLayerType() == SDL::Module::Pixel) ?  (outerLowerModule.partnerModule())->getDrDz() : outerLowerModule.getDrDz();
    ...
    {
        dAlphaValues[0] = dAlpha_Bfield + sqrt(pow(dAlpha_res,2) + pow(sdMuls,2));
    }
    else
    {
        dAlphaValues[0] = dAlpha_Bfield + sqrt(pow(dAlpha_res,2) + pow(sdMuls,2) + pow(sdLumForInnerMini,2));
    }

    if (outerLowerModule.subdet() == SDL::Module::Barrel and outerLowerModule.side() == SDL::Module::Center)
    {
        dAlphaValues[1] = dAlpha_Bfield + sqrt(pow(dAlpha_res,2) + pow(sdMuls,2));
    }
    else
    {
        dAlphaValues[1] = dAlpha_Bfield + sqrt(pow(dAlpha_res,2) + pow(sdMuls,2) + pow(sdLumForOuterMini,2));
    }

    //Inner to outer 
    dAlphaValues[2] = dAlpha_Bfield + sqrt(pow(dAlpha_res,2) + pow(sdMuls,2));
